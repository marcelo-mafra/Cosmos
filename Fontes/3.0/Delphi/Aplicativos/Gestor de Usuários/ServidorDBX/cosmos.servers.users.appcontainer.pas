unit cosmos.servers.users.appcontainer;
...
var
  DMUsuariosAppContainer: TDMUsuariosAppContainer;
...
uses Winapi.Windows, cosmos.servers.common.services, cosmos.servers.common.methods,
  ...
  cosmos.servers.common.security.authorizations, cosmos.servers.users.usersmethods;
...
procedure TDMUsuariosAppContainer.DSTCPServerTransportConnect(
...
procedure TDMUsuariosAppContainer.DSTCPServerTransportDisconnect(
  Event: TDSTCPDisconnectEventObject);
begin
...
procedure TDMUsuariosAppContainer.GetServerConfigurations(
  AList: TStringList);
begin
 ...
 try
  for I := 0 to Pred(IAuthorizations.Methods.Count) do
   begin
    ...
    if (IMethodInfo.CosmosModule = 'cmUsuarios') or (IMethodInfo.CosmosModule = 'cmAll') then //do not localize!
     begin
 ...
 try
  ...
  if UpperCase(CurrentProtocol) = 'TCP/IP' then //Conexão TCP/IP
   begin
    ...
    DSTCPServerTransport.Server := DSServerUsuarios;
  ...
  if UpperCase(CurrentProtocol) = 'HTTP' then  //Configurações do objeto de conexão HTTP
   begin
    ...
    DSHTTPService.Server := DSServerUsuarios;
    DSHTTPService.Start;
...
procedure TDMUsuariosAppContainer.ValidateCertificate(Owner: TObject;
...
begin
 ...
 //outputdebugstring(PWideChar(DMCosmosServerServices.HashValue('326612')));
...
procedure TDMUsuariosAppContainer.DSCertFilesGetPEMFilePasskey(ASender: TObject;
  var APasskey: AnsiString);
begin
  APasskey := '';
end;
...
procedure TDMUsuariosAppContainer.DSHTTPServiceTrace(Sender: TObject;
