unit cosmos.servers.focos.appcontainer;
...
type
  TServerProtocol = (spTCP, spHTTPS); //Protocolos de comenxão que podem ser usados.
  ...
  TDMFocosAppContainer = class(TDataModule)
    DSServerFocos: TDSServer;
    DSTCPServerTransport: TDSTCPServerTransport;
    DSHTTPService: TDSHTTPService;
    DSCertFiles: TDSCertFiles;
    DSAuthenticationManager: TDSAuthenticationManager;
    DSServerApplicationClass: TDSServerClass;
    DSServerFocosClass: TDSServerClass;
    ...
    procedure DSServerFocosPrepare(
      DSPrepareEventObject: TDSPrepareEventObject);
    procedure DSServerFocosError(DSErrorEventObject: TDSErrorEventObject);
    procedure DSServerFocosConnect(
      DSConnectEventObject: TDSConnectEventObject);
    procedure DSServerFocosDisconnect(
      DSConnectEventObject: TDSConnectEventObject);
    function DSServerFocosTrace(TraceInfo: TDBXTraceInfo): CBRType;
    ...
    procedure DSServerFocosClassGetClass(DSServerClass: TDSServerClass;
      var PersistentClass: TPersistentClass);
  private
...
var
  DMFocosAppContainer: TDMFocosAppContainer;
...
uses Winapi.Windows, cosmos.servers.common.services, cosmos.servers.common.methods,
  ...
  cosmos.servers.common.security.authorizations, cosmos.servers.focos.methods;


{$R *.dfm}

procedure TDMFocosAppContainer.DSServerApplicationClassGetClass(
  DSServerClass: TDSServerClass; var PersistentClass: TPersistentClass);
begin
  PersistentClass := TDMCosmosApplicationServer;
end;

procedure TDMFocosAppContainer.DSServerFocosClassGetClass(
  DSServerClass: TDSServerClass; var PersistentClass: TPersistentClass);
begin
  PersistentClass := TDMCosmosConferenciasMethods;
end;

procedure TDMFocosAppContainer.DSServerFocosConnect(
  DSConnectEventObject: TDSConnectEventObject);
var
 AInfo: string;
...
procedure TDMFocosAppContainer.DSServerFocosDisconnect(
  DSConnectEventObject: TDSConnectEventObject);
var
 AInfo: string;
 AContextInfo: TStringList;
...
procedure TDMFocosAppContainer.DSServerFocosError(
...
procedure TDMFocosAppContainer.DSServerFocosPrepare(
  DSPrepareEventObject: TDSPrepareEventObject);
var
 Info: string;
 AContextInfo: TStringList;
...
function TDMFocosAppContainer.DSServerFocosTrace(
...
procedure TDMFocosAppContainer.DSTCPServerTransportConnect(
...
procedure TDMFocosAppContainer.DSTCPServerTransportDisconnect(
  Event: TDSTCPDisconnectEventObject);
begin
...
procedure TDMFocosAppContainer.GetServerConfigurations(
  AList: TStringList);
begin
...
procedure TDMFocosAppContainer.LoadMethodsAuthorizations;
var
 ...
 try
  for I := 0 to Pred(IAuthorizations.Methods.Count) do
   begin
    ...
    if (IMethodInfo.CosmosModule = 'cmFocos') or (IMethodInfo.CosmosModule = 'cmAll') then //do not localize!
     begin
...
procedure TDMFocosAppContainer.LoadServerConfigurations;
 var
...
{Carregas as configurações do servidor em execução. Estas configurações envolvem
 ...
 try
  ...
  DSServerApplicationClass.LifeCycle := AFile.ReadString('GFOC', 'DSServerApplicationClass.LifeCycle', 'Session');
  DSServerFocosClass.LifeCycle := DSServerApplicationClass.LifeCycle;

  CurrentProtocol := AFile.ReadString('GFOC', 'DSCurrentProtocol', 'HTTPS');

  if UpperCase(CurrentProtocol) = 'TCP/IP' then //Conexão TCP/IP
   begin
    DSTCPServerTransport.BufferKBSize := AFile.ReadInteger('GFOC', 'TCPConnection.BufferKBSize', 32);
    DSTCPServerTransport.Port := AFile.ReadInteger('GFOC', 'TCPConnection.Port', 10001);
    DSTCPServerTransport.MaxThreads := AFile.ReadInteger('GFOC', 'TCPConnection.MaxThreads', 0);
    DSTCPServerTransport.PoolSize := AFile.ReadInteger('GFOC', 'TCPConnection.PoolSize', 10);

    case AFile.ReadInteger('GFOC', 'TCPConnection.KeepAliveEnablement', 0) of
    ...
    DSTCPServerTransport.KeepAliveTime := AFile.ReadInteger('GFOC', 'TCPConnection.KeepAliveTime', 0);
    DSTCPServerTransport.KeepAliveInterval := AFile.ReadInteger('GFOC', 'TCPConnection.KeepAliveInterval', 0);

    DSTCPServerTransport.Server := DSServerFocos;
    DSTCPServerTransport.Start;
   end
   else
  if UpperCase(CurrentProtocol) = 'HTTP' then  //Configurações do objeto de conexão HTTP
   begin
    sTestServerPage := AFile.ReadString('GFOC', 'TestSeverPage', '');
    DSHTTPService.DSHostname := AFile.ReadString('GFOC', 'DSHTTPService.DSHostname', '');
    DSHTTPService.DSPort := AFile.ReadInteger('GFOC', 'DSHTTPService.DSPort', 0);
    DSHTTPService.HttpPort := AFile.ReadInteger('GFOC', 'DSHTTPService.HttpPort', 0);
    DSHTTPService.SessionTimeout := AFile.ReadInteger('GFOC', 'DSHTTPService.SessionTimeout', 0);

    DSHTTPService.Server := DSServerFocos;
    DSHTTPService.Start;
  ...
  if UpperCase(CurrentProtocol) = 'HTTPS' then  //Configurações do objeto de conexão HTTPS
   begin
    ...
    DSHTTPService.DSHostname := AFile.ReadString('GFOC', 'DSHTTPService.DSHostname', '');
    DSHTTPService.DSPort := AFile.ReadInteger('GFOC', 'DSHTTPService.DSPort', 0);
    DSHTTPService.HttpPort := AFile.ReadInteger('GFOC', 'DSHTTPService.HttpsPort', 443);
    DSHTTPService.SessionTimeout := AFile.ReadInteger('GFOC', 'DSHTTPService.SessionTimeout', 0);
...
procedure TDMFocosAppContainer.ValidateCertificate(Owner: TObject;
...
procedure TDMFocosAppContainer.DataModuleCreate(Sender: TObject);
begin
...
procedure TDMFocosAppContainer.DSAuthenticationManagerUserAuthenticate(
...
begin
    ...
    begin
     ...
     if Valid then
      begin
       ...
       //Agora registra logs sobre a autneticação.
 ...
 AUserInfo.Free;
  ...
  //o usuário não pode acessar o módulo do Cosmos.
...
procedure TDMFocosAppContainer.DSAuthenticationManagerUserAuthorize(
...
procedure TDMFocosAppContainer.DSCertFilesGetPEMFilePasskey(ASender: TObject;
  var APasskey: AnsiString);
begin
  APasskey := '';
end;
...
procedure TDMFocosAppContainer.DSHTTPServiceTrace(Sender: TObject;
