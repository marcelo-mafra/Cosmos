unit cosmos.servers.conferencias.appcontainer;
...
uses Winapi.Windows, cosmos.servers.common.services, cosmos.servers.common.methods,
  ...
  cosmos.servers.common.security.authorizations, cosmos.servers.conferencias.methods,
  cosmos.servers.secretarias.atividades.methods;
...
procedure TDMConferenciasAppContainer.DSTCPServerTransportConnect(
...
procedure TDMConferenciasAppContainer.DSTCPServerTransportDisconnect(
  Event: TDSTCPDisconnectEventObject);
begin
...
procedure TDMConferenciasAppContainer.GetServerConfigurations(
  AList: TStringList);
begin
 ...
 try
  for I := 0 to Pred(IAuthorizations.Methods.Count) do
   begin
    ...
    if (IMethodInfo.CosmosModule = 'cmConferencias') or (IMethodInfo.CosmosModule = 'cmAll') then //do not localize!
     begin
...
{Carregas as configurações do servidor em execução. Estas configurações envolvem
 ...
 try
  ...
  CurrentProtocol := AFile.ReadString('GCON', 'DSCurrentProtocol', 'HTTPS');
  ...
  if UpperCase(CurrentProtocol) = 'TCP/IP' then //Conexão TCP/IP
   begin
    ...
    DSTCPServerTransport.Port := AFile.ReadInteger('GCON', 'TCPConnection.Port', 10001);
    ...
    case AFile.ReadInteger('GCON', 'TCPConnection.KeepAliveEnablement', 0) of
    ...
    DSTCPServerTransport.KeepAliveTime := AFile.ReadInteger('GCON', 'TCPConnection.KeepAliveTime', 0);
    DSTCPServerTransport.KeepAliveInterval := AFile.ReadInteger('GCON', 'TCPConnection.KeepAliveInterval', 0);

    DSTCPServerTransport.Server := DSServerConferencias;
    DSTCPServerTransport.Start;
   end
   else
  if UpperCase(CurrentProtocol) = 'HTTP' then  //Configurações do objeto de conexão HTTP
   begin
    sTestServerPage := AFile.ReadString('GCON', 'TestSeverPage', '');
    DSHTTPService.DSHostname := AFile.ReadString('GCON', 'DSHTTPService.DSHostname', '');
    DSHTTPService.DSPort := AFile.ReadInteger('GCON', 'DSHTTPService.DSPort', 0);
    DSHTTPService.HttpPort := AFile.ReadInteger('GCON', 'DSHTTPService.HttpPort', 0);
    DSHTTPService.SessionTimeout := AFile.ReadInteger('GCON', 'DSHTTPService.SessionTimeout', 0);

    DSHTTPService.Server := DSServerConferencias;
    DSHTTPService.Start;
  ...
  if UpperCase(CurrentProtocol) = 'HTTPS' then  //Configurações do objeto de conexão HTTPS
   begin
    ...
    DSHTTPService.DSHostname := AFile.ReadString('GCON', 'DSHTTPService.DSHostname', '');
    DSHTTPService.DSPort := AFile.ReadInteger('GCON', 'DSHTTPService.DSPort', 0);
    DSHTTPService.HttpPort := AFile.ReadInteger('GCON', 'DSHTTPService.HttpsPort', 443);
    DSHTTPService.SessionTimeout := AFile.ReadInteger('GCON', 'DSHTTPService.SessionTimeout', 0);
...
procedure TDMConferenciasAppContainer.ValidateCertificate(Owner: TObject;
...
procedure TDMConferenciasAppContainer.DSCertFilesGetPEMFilePasskey(ASender: TObject;
  var APasskey: AnsiString);
begin
  APasskey := '';
end;
...
procedure TDMConferenciasAppContainer.DSHTTPServiceTrace(Sender: TObject;
