...
constructor TFastMemStream.Create(InitialCapacity: Integer);
begin
  inherited Create;
...
constructor TBufferWriter.Create(ASource: TStream; AChunkSize: integer);
begin
  inherited Create;
  FSource := ASource;
...
function TBufferWriter.Read(var Buffer; Count: Integer): Longint;
begin
  // not implemented
  raise Exception.Create('not implemented');
end;

function TBufferWriter.Write(const Buffer; Count: Integer): Longint;
var
  Idx, Siz: integer;
begin
...
procedure TBufferWriter.WriteChunk(Count: integer);
begin
  if Count > 0 then
  begin
