...
const
  ...
  SInvalidDest = 'O destino %s não existe';
  SFCantMove = 'Não foi possível mover o arquivo %s';
...
begin
  ...
  LnkName := TShellFolders.GetSpecialFolderPath(Dest, True) + '\' +
...
var //define binary exe type
  BinaryType: DWORD;
begin
  If GetBinaryType(Pchar(Filename), Binarytype) Then
    case BinaryType of
      SCS_32BIT_BINARY: Result:= 'Executável do Win32';
      SCS_DOS_BINARY  : Result:= 'Executável do DOS';
      SCS_WOW_BINARY  : Result:= 'Executável do Win16';
      SCS_PIF_BINARY  : Result:= 'Arquivo PIF';
      SCS_POSIX_BINARY: Result:= 'Executável POSIX';
      SCS_OS216_BINARY: Result:= 'Executável OS/2 16 bits'
    else
      Result:= 'Executável desconhecido'
    end
else
  Result:= 'O arquivo não é um executável';
end;
...
Command: string;
begin
 Command := 'rundll32.exe shell32.dll,Control_RunDLL ' + AppletPath;
...
begin
  ...
  begin
    try
       ...
       result := 'O endereço de IP é inválido!';
       exit;
    end;
  end;
...
begin
 if GetOperatingSystem <= cOsSeven then
  ShellExecute(Application.Handle, 'open', PWideChar(prog), PWideChar(params), nil, SW_NORMAL)
 else
  begin
...
begin
  StrPCopy(c, FileName);
  ShellExecute(Application.Handle, 'print', c, nil, nil, SW_NORMAL);
end;
...
// Returna True o usuário está usnado o estilo XP
const
...
//Testa se uma pasta está vazia
  try
   Result := (FindFirst(FolderName + '\*.*', faAnyFile, SearchRec) = 0) and
...
begin
  ...
  try
    ...
    begin
      ...
      cmd    := StringReplace(cmd, '%1', Folder, []);
      Result := True;
...
begin
 ShellExecute(Handle, PChar('Open'), PChar('C:\Windows\Rundll32.exe'),
...
begin
  ...
  ShellExecute(Handle, 'print', PChar(FileName), PChar(S), nil, SW_HIDE);
...
begin
 Result := (Host.Trim = '127.0.0.1') or (Host.LowerCase(Host) = 'localhost');
end;
