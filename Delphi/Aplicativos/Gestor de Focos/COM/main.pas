...
uses
  ...
  ImgList, XPMan,  cosmos.framework.forms.mydialogs;
...
uses cosmos.core.winshell, cosmos.core.ConstantesMsg, FormAdmLogin, cosmos.core.cripter;
...
begin
 ...
 TrayIcon.Hint := TrayIcon.BalloonHint;
...
begin
 if TMyDialogs.ConfirmMessage(Format(sConfShutDownWindows, [Application.Title,
   Application.Title, CManager.ConnectionsCount])) = mrNo then
  Msg.Result := 0
 else
  Msg.Result := 1;
end;
 ...
 try
  ...
  with Cript do
   begin
    Key := FCodeKey;
    Model := cmRDL256;
    CManager.AdminLogon :=  '';//Encrypt('');
    CManager.AdminPassword := '';//Encrypt('');
   end;
