...
begin
 with CdsDiscipulados do
  begin
   Filtered := False;
   Filter := Format(TFilterData.CAMDIS_INDATI, [QuotedStr('LEC'), QuotedStr('S')]);
   Filtered := True;
...
begin
  ...
  CdsDiscipulados.Filter := Format(TFilterData.CAMDIS_INDESCINT_INDATI,[QuotedStr('LEC'), QuotedStr('N'), QuotedStr('S')]);
...
begin
  CdsDiscipulados := IRemoteCon.OpenBufferedData(TCosmosFiles.BufDiscipulados);
  CdsDiscipulados.Filtered := False;
  CdsDiscipulados.Filter := Format(TFilterData.INDESCINT_INDATI,[QuotedStr('S'), QuotedStr('S')]);
  CdsDiscipulados.Filtered := True;
  CdsDiscipulados.IndexFieldNames := 'seqdis';
