 ...
 try
  ...
  for I := 0 to Integer(ctAll) do
   begin
    ...
    if Campo = ctTP then
     CdsDiscipulados.Filter := Format(TFilterData.CAMDIS_INDATI,[QuotedStr('TPU'), QuotedStr('S')])
    else
    if Campo = ctTM then
     CdsDiscipulados.Filter := Format(TFilterData.CAMDIS_INDATI,[QuotedStr('TMO'), QuotedStr('S')])
    else
    if Campo = ctLectorium then
     CdsDiscipulados.Filter := Format(TFilterData.CAMDIS_INDATI,[QuotedStr('LEC'), QuotedStr('S')])
    else
    if Campo = ctTMB then
     CdsDiscipulados.Filter := Format(TFilterData.CAMDIS_INDATI,[QuotedStr('TMB'), QuotedStr('S')])
    else
     Continue;
