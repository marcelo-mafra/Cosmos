object FrmDesligamento: TFrmDesligamento
  Left = 0
  Top = 0
  HelpContext = 375
  BorderStyle = bsDialog
  Caption = 'Desligamento'
  ClientHeight = 489
  ClientWidth = 497
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  DesignSize = (
    497
    489)
  PixelsPerInch = 96
  TextHeight = 13
  object MSGroupHeader1: TMSGroupHeader
    Left = 8
    Top = 8
    Width = 481
    Height = 129
    Transparent = True
    Anchors = [akLeft, akTop, akRight]
    Caption = 'Cadastrado'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    DesignSize = (
      481
      129)
    object Label6: TLabel
      Left = 0
      Top = 26
      Width = 27
      Height = 13
      Caption = '&Nome'
      FocusControl = EdtNome
    end
    object Label7: TLabel
      Left = 0
      Top = 77
      Width = 53
      Height = 13
      Caption = '&Discipulado'
      FocusControl = EdtDiscipulado
    end
    object Label8: TLabel
      Left = 148
      Top = 77
      Width = 23
      Height = 13
      Caption = '&Foco'
      FocusControl = EdtFoco
    end
    object EdtNome: TEdit
      Left = 0
      Top = 41
      Width = 481
      Height = 21
      Anchors = [akLeft, akTop, akRight]
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 0
    end
    object EdtDiscipulado: TEdit
      Left = 0
      Top = 94
      Width = 113
      Height = 21
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 1
    end
    object EdtFoco: TEdit
      Left = 148
      Top = 94
      Width = 333
      Height = 21
      Anchors = [akLeft, akTop, akRight]
      Color = clBtnFace
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 2
    end
  end
  object MSGroupHeader2: TMSGroupHeader
    Left = 8
    Top = 146
    Width = 481
    Height = 308
    Transparent = True
    Anchors = [akLeft, akTop, akRight, akBottom]
    Caption = 'Hist'#243'rico Discipular'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    DesignSize = (
      481
      308)
    object Label1: TLabel
      Left = 0
      Top = 80
      Width = 23
      Height = 13
      Caption = '&Data'
    end
    object Label2: TLabel
      Left = 0
      Top = 29
      Width = 114
      Height = 13
      Caption = '&Motivo do Desligamento'
      FocusControl = CBXMotivo
    end
    object Label3: TLabel
      Left = 0
      Top = 124
      Width = 63
      Height = 13
      Caption = '&Observa'#231#245'es'
      FocusControl = MmoObservacoes
    end
    object Label5: TLabel
      Left = 163
      Top = 80
      Width = 57
      Height = 13
      Caption = '&Falecimento'
      FocusControl = EdtDataFalecimento
    end
    object CBXMotivo: TComboBox
      Left = 0
      Top = 45
      Width = 481
      Height = 21
      Style = csDropDownList
      TabOrder = 0
      TextHint = 'Selecione o motivo do desligamento...'
      OnChange = CBXMotivoChange
      Items.Strings = (
        'A pedido'
        'Infreq'#252#234'ncia'
        'Falecimento'
        'Transfer'#234'ncia para o exterior')
    end
    object EdtDataDesligamento: TDateTimePicker
      Left = 0
      Top = 95
      Width = 126
      Height = 21
      Date = 41632.721194074070000000
      Time = 41632.721194074070000000
      TabOrder = 1
    end
    object EdtDataFalecimento: TDateTimePicker
      Left = 163
      Top = 95
      Width = 126
      Height = 21
      Date = 41632.721194074070000000
      Time = 41632.721194074070000000
      TabOrder = 2
    end
    object MmoObservacoes: TMemo
      Left = 0
      Top = 141
      Width = 481
      Height = 161
      Anchors = [akLeft, akTop, akRight, akBottom]
      ScrollBars = ssVertical
      TabOrder = 3
    end
  end
  object BitBtn1: TBitBtn
    Left = 230
    Top = 456
    Width = 75
    Height = 25
    Anchors = [akRight, akBottom]
    Caption = '&OK'
    Glyph.Data = {
      
      FFFF00FFFF00FFFF00FFFF00FF00}
    ModalResult = 1
    TabOrder = 2
  end
  object BitBtn2: TBitBtn
    Left = 324
    Top = 456
    Width = 75
    Height = 25
    Anchors = [akRight, akBottom]
    Caption = '&Cancelar'
    Glyph.Data = {
      
      0000}
    ModalResult = 2
    TabOrder = 3
  end
  object BitBtn3: TBitBtn
    Left = 414
    Top = 456
    Width = 75
    Height = 25
    Anchors = [akRight, akBottom]
    Caption = '&Ajuda'
    Glyph.Data = {
      
      FF00FF00FF00FF00FF00}
    TabOrder = 4
    OnClick = BitBtn3Click
  end
end
